---
layout: single
title: Configuring tslint with Vuetify
...
These don't look great, so let's make them go away.
...
You just need to configure [tslint]'s `rules`.

Add the following settings to your `tslint.json` file.
...
Of course, you don't have to configure `tslint` exactly like this. Feel free to set it up however you prefer (?).

Don't forget to run `npm run lint --fix` as well.
