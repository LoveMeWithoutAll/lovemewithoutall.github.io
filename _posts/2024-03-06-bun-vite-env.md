---
layout: single
title: bun + vite build 에서 환경변수 설정이 되지 않는 오류 해결
...
# 현상
bun을 runtime으로 하여 vite build 명령을 실행하면 환경변수 문제가 발생한다. 보통 `.env`를 사용하여 환경별로 여러 개의 `.env` 파일을 만든다.
...
* bun은 그냥 아무 응답이 없다([issue link](https://github.com/oven-sh/bun/issues/6338_))
* vite에서는 bun의 문제라며 [외면 중](https://github.com/vitejs/vite/issues/14912)이다
* bun의 이 문제에 대한 해결책을 제시하는 사람이 아무도 없어서... 그냥 내가 [우회책을 달아놓았다.](https://github.com/oven-sh/bun/issues/6338#issuecomment-1980085235)
